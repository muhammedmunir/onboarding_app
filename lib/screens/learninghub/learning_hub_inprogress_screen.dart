...
class LearningHubInprogressScreen extends StatefulWidget {
  ...
  @override
  State<LearningHubInprogressScreen> createState() =>
      _LearningHubInprogressScreenState();
}

class _LearningHubInprogressScreenState
    extends State<LearningHubInprogressScreen> with WidgetsBindingObserver {
  final TextEditingController _searchController = TextEditingController();
  bool _shouldRefresh = false;
  late Stream<List<Map<String, dynamic>>> _inprogressLearningsStream;
  ...
  void _setupStream() {
    ...
    if (user != null) {
      _inprogressLearningsStream = _getInprogressLearningsStream(user.uid);
    } else {
      _inprogressLearningsStream = Stream.value([]);
    }
  }
  ...
  Stream<List<Map<String, dynamic>>> _getInprogressLearningsStream(
      String userId) {
    ...
    return allLearningsStream.asyncMap((querySnapshot) async {
      final futures = querySnapshot.docs.map((doc) {
        return _getLearningWithUserProgressIfInprogress(doc, userId);
      }).toList();
  ...
  Future<Map<String, dynamic>?> _getLearningWithUserProgressIfInprogress(
            ...
            children: [
              ...
              Container(
                ...
                child: TextField(
                  ...
                  decoration: const InputDecoration(
                    hintText: 'Search In Progress Courses...',
              ...
              Expanded(
                child: StreamBuilder<List<Map<String, dynamic>>>(
                  stream: _inprogressLearningsStream,
                  builder: (context, snapshot) {
                    ...
                    final allInprogressCourses = snapshot.data ?? [];

                    // Terapkan filter pencarian
                    final query = _searchController.text.trim().toLowerCase();
                    final filteredCourses = allInprogressCourses.where((course) {
